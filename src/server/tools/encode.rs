...
impl Server {
    ...
    fn base64_encode(&self, Parameters(params): Parameters<Base64EncodeDecodeParams>) -> String {
        STANDARD.encode(params.data.as_bytes())
    }
    ...
    fn url_encode(&self, Parameters(params): Parameters<UrlEncodeDecodeParams>) -> String {
        form_urlencoded::byte_serialize(params.url.as_bytes()).collect()
    }
    ...
    fn url_decode(&self, Parameters(params): Parameters<UrlEncodeDecodeParams>) -> String {
        form_urlencoded::parse(params.url.as_bytes())
            .map(|(k, v)| {
                if v.is_empty() {
                    k.to_string()
                } else {
    ...
    #[tool(name = "encode.hex_to_string", description = "Hex data to utf8 string")]
    fn hex_to_string(
        &self,
